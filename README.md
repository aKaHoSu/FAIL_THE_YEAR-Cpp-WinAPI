...
- ゲームジャンル：落下物回避ゲーム
- コンセプト：「留年を回避して進級する」
- ゲームのスクリーンショットやデモ動画のリンクは現在ありません。追加予定です。

--- 

## 2. ゲームの遊び方
- 操作方法：十字キー移動 ・ Shift 押しっぱで低速 ・ Z でボム）
- 上から降ってくる「留年」を一定数回避するとクリア
- 難易度：Easy / Hard / Nightmare）

--- 

## 3. 使用技術・開発環境
- 言語：C++
- API：Windows API（GDI による自前描画）
...
### - ダブルバッファリングの自前実装
GDI の `CreateCompatibleDC` を用いたバックバッファへの描画を実装し、画面のちらつきを防止しています。
